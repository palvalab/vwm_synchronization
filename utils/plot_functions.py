...
def plot_lines(data, names=None, figsize=[13,2], cmap='jet', colors = None, 
               ...
               xticklabels = None, title = None, less_spines = True, 
    ...
    for i in range(N_plots):    
       ...
       # ax.hold(True)
        ...
        if np.all(zticks[i] != None):
            cb = plt.colorbar(p, cax=cax, ticks = zticks[i], format=zformat)  
...
def plot_heatmaps_pruned(data, titles=None, N_cols=3, figsize=None, fontsizeT=13, fontsizeL=11, 
                  fontsizeS=11, ylabel=None, xlabel=None, zlabel= None, cmap='jet',zmax=None, zmin=0,
                  xticks = None, yticks = None, zticks = None,
                  xticklabels = None, yticklabels=None, zticklabels=None,xticklab_rot=0,
                  zformat = '%0.3f'):
    ''' Input:
    ...
    for i in range(N_plots):    
       ...
       # ax.hold(True)
        ...
        if zmax[i] == None:
            zmax[i] = np.max(data[i])
            if zmax[i] < 0:
                zmin[i] = np.min(data[i])
        p = ax.imshow(data[i],origin='lower',interpolation='none',cmap=cmaps[i],vmax=zmax[i],vmin=zmin[i])
        divider = make_axes_locatable(ax)
        
        #if i in range(N_cols-1, N_plots, N_cols):
        cax = divider.append_axes("right", size="5%", pad=0.1)
        if np.all(zticks[i] != None):
            cb = plt.colorbar(p, cax=cax, ticks = zticks[i], format=zformat)  
            if zticklabels[i] != None:
        ...
        if np.all(ylabel!=None):    
            ax.set_ylabel(ylabel[i], fontsize=fontsizeL)   
        if np.all(zlabel!=None):    
            cb.ax.set_title(zlabel[i], fontsize=fontsizeS)  
        if np.all(xticks[i] !=None):
            ax.set_xticks(xticks[i])
        if np.all(yticks[i] !=None):
            ax.set_yticks(yticks[i])
...
def semi_log_plot(figsize,data,freqs,xlim=[1,100],ylabel='variable',legend=None,outfile=None,
...
def _depth(obj,c=0):
  ...
  #
#
#def violinplots(figsize,data,freqs,outfile):
#    
...
#    
#    
#def log_log_plot(figsize,data,freqs,xlim,ylab,legend=None,outfile=None,legend_pos=None,ylim=None,show=False):  
#    plt.figure(figsize=figsize, dpi=600, facecolor='w', edgecolor='k')
#    plt.loglog(freqs,data)
#    plt.xlim(xlim) 
...
#        plt.ylim(ylim) 
#    if legend_pos=='uc':
#        plt.legend(legend,loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=4)
#    if legend_pos=='ur':
...
#      
    ...
    #def plot_heatmap_2(fig,pos,data,xlab,ylab,xtix,ytix,zmax='None',CM=plt.cm.YlOrRd,                         # for Pacman
#                 masking='False',bad='white',under='None',topaxis='True',xlab_rot=45,
#                 cbar='True'):
#    
#    eps = np.spacing(0.0)
#    #fig, ax = plt.subplots(1,figsize=figsize)
...
#
#        
#    PCM =  ax.pcolormesh(data,vmin=eps,vmax=zmax,cmap=CM)    
#    ax.set_xticks(np.arange(Nx)+0.5)
#    ax.set_xticklabels(xtix,rotation=xlab_rot)
#    ax.set_xlim([0,Nx])      
...
#    ax.tick_params(axis='both',which='both',length=0)
#    if cbar:
#    #    PCM = ax.get_children()[2]          #get the mappable, 
